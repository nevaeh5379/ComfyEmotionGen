...
export function getCategoryStyle(classType: string): CategoryStyle {
  ...
  if (
    ...
    return CATEGORY_STYLES.loader
  if (lower.includes("sampler") || lower.includes("ksampler"))
    return CATEGORY_STYLES.sampler
  if (
    lower.includes("encode") ||
    lower.includes("conditioning") ||
    lower.includes("clip")
  )
    return CATEGORY_STYLES.conditioning
  if (lower.includes("vae") || lower.includes("latent"))
    return CATEGORY_STYLES.latent
  if (
    lower.includes("image") ||
    lower.includes("preview") ||
    lower.includes("save")
  )
    return CATEGORY_STYLES.image
