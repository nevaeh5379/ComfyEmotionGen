...
export function ImageViewer({
  ...
  const getImageRect = useCallback((z?: number) => {
    ...
    const zoomVal = z ?? zoomRef.current
    const p = panRef.current
    const imgW = baseW * zoomVal
    const imgH = baseH * zoomVal
    return {
      cw,
      ch,
      imgW,
  ...
  const zoomToRegion = useCallback(
    (x1: number, y1: number, x2: number, y2: number) => {
      ...
      const selW = (rx2 - rx1) * imgNatural.w * fitScale
      const selH = (ry2 - ry1) * imgNatural.h * fitScale
      ...
      const newPan = clampPan(newZoom, {
        x: (0.5 - cxPct) * newZoom * imgNatural.w * fitScale,
        y: (0.5 - cyPct) * newZoom * imgNatural.h * fitScale,
      })
      setZoomAndRef(newZoom)
      setPanAndRef(newPan)
    },
    [clampPan, setZoomAndRef, setPanAndRef, imgNatural, getImageRect]
  )
      ...
      if (shiftSelectRef.current) {
        if (dist > CLICK_THRESHOLD) {
          ...
          if (el) {
            const rect = el.getBoundingClientRect()
            zoomIn({
              clientX: dragStartRef.current.x + rect.left,
              clientY: dragStartRef.current.y + rect.top,
            })
          }
        }
  ...
  const handleMouseDown = useCallback(
    ...
    [setDragging, setShiftSelect, setDragStart, setDragCurrent]
  )
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      // magnifier lens
      setLensScreenPos({
        x: e.clientX + LENS_OFFSET,
        y: e.clientY + LENS_OFFSET,
      })
      const r = getImageRect()
      if (r) {
  ...
  }, [dragging, onWindowMouseMove, onWindowMouseUp])
