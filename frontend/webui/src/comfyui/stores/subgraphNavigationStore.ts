...
export const useSubgraphNavigationStore = create<SubgraphNavigationState>(
  (set): SubgraphNavigationState => ({
    activeSubgraph: null,
    idStack: [],
    ...
    navigateToRoot: (): void => {
      ...
      nextViewportCache.set(cacheKey, {
        pan: [...graphStore.pan] as [number, number],
    ...
    restoreStack: (idStack: SubgraphId[]): void => {
      useReactGraphStore.setState({
        activeGraphId:
          idStack.length > 0 ? (idStack[idStack.length - 1] ?? null) : null,
...
function get(): SubgraphNavigationState {
  return useSubgraphNavigationStore.getState()
}
