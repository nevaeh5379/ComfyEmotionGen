...
export const useSubgraphBlueprintStore = create<SubgraphBlueprintState>(
  (set, get): SubgraphBlueprintState => ({
    blueprints: [],
    globalBlueprints: [],
    isLoading: false,
    ...
    publishSubgraph: (name: string): { success: boolean; error?: string } => {
      ...
      if (!model)
        return { success: false, error: "Subgraph 정의를 찾을 수 없습니다" }

      // 내부 노드/링크 추출
      const innerNodes = graphStore.nodes.filter((n) => n.graphId === node.type)
      const innerLinks = graphStore.links.filter(
        (l) =>
          l.origin_id === -10 ||
          l.target_id === -20 ||
    ...
    instantiateBlueprint: (
      ...
      if (!meta)
        return { success: false, error: "블루프린트를 찾을 수 없습니다" }
...
export async function loadGlobalSubgraphDefinition(
  id: string
