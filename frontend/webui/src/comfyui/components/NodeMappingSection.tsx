  ...
  ({
    ...
    const importableWorkflows = useMemo(() => {
      return savedWorkflows.filter(
        (w: SavedWorkflow) => w.mappingPresets.length > 0
      )
    }, [savedWorkflows])

    const safeImageUploads = imageUploads ?? {}
    const safeHandleImageUpload: (
      file: File,
      nodeId: string,
      inputKey: string
    ) => void =
      handleImageUpload ??
      ((_file: File, _nodeId: string, _inputKey: string): void => {
        return
      })
      ...
      <CollapseSection
        ...
        meta={
          nodeMappings.length > 0
            ? `${String(nodeMappings.length)}개 매핑`
            : undefined
        }
      >
            ...
            {activeWorkflowId !== null ? (
              ...
              <div className="flex items-center justify-between rounded-md border bg-muted/40 px-3 py-1.5">
                ...
                {importableWorkflows.length > 0 && onImportFromPreset && (
                  <DropdownMenu
                    open={showImportMenu}
                    onOpenChange={setShowImportMenu}
                  >
                    ...
                    <TableBody>
                      {nodeMappings.map((m) => {
                        ...
                        const enumOptions = Array.isArray(spec?.[0])
                          ...
                          : null
                        const upload =
                          safeImageUploads[`${m.nodeId}.${m.inputKey}`]
                        return (
                          <TableRow
                            key={m.id}
